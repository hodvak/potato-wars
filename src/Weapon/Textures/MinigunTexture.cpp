...
MinigunTexture::MinigunTexture(PlayerColor color,
...
void MinigunTexture::setPosition(const MapVector &position)
{
    m_position = position;
}

void MinigunTexture::setAimPosition(const MapVector &aimPosition)
{
    m_aimPosition = aimPosition;
}

void MinigunTexture::draw(sf::RenderTarget &target, sf::RenderStates states) const
{
    sf::Sprite aim;
    ...
    sprite.setTextureRect(sf::IntRect(
            left * (int) resources_manager::IMG_MINIGUN_SIZE.x,
            0,
            (int) resources_manager::IMG_MINIGUN_SIZE.x,
            (int) resources_manager::IMG_MINIGUN_SIZE.y
    ));

    sprite.setOrigin(
            (float) resources_manager::IMG_MINIGUN_SIZE.x / 2.0f,
            (float) resources_manager::IMG_MINIGUN_SIZE.y / 2.0f
    );
    sprite.setScale(
            m_radius / ((float) resources_manager::IMG_MINIGUN_SIZE.x / 2.0f),
            m_radius / ((float) resources_manager::IMG_MINIGUN_SIZE.x / 2.0f)
    );
